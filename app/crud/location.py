...
class LocationRepository:
    def get(self, db: Session, location_id: int):
    ...
    @staticmethod
    def search(self, db: Session, term: str, skip: int = 0, limit: int = 100):
        """Search locations by name or address"""
        return db.query(Location).filter(or_(
            Location.name.ilike(f"%{term}%"),
            Location.address.ilike(f"%{term}%")
        )).offset(skip).limit(limit).all()

    @staticmethod
    def get_with_stock_count(self, db: Session, location_id: int):
        """
        ...
        stock_count = db.query(
            ...
            .filter(InventoryItem.location_id == location.id).scalar()
            .scalar()
        )
