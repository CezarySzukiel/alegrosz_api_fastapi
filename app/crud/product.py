...
class ProductRepository:
    ...
    def search(db: Session, term: str, skip: int = 0, limit: int = 100) -> list[type[Product]]:
        return db.query(Product).filter(or_(
    ...
    @staticmethod
    def filter_by_price(db: Session, min_price: float | None, max_price: float | None, skip: int = 0,
    ...
    @staticmethod
    def create(db: Session, name: str, sku: str, description: str, price: float) -> Product:
        product = Product(
            name=name,
            sku=sku,
